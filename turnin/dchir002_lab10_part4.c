...
const unsigned long pG = 1;
const unsigned long p1 = 300; //bit 0-3 lights
const unsigned long p2 = 1000; //bit 4 light
const unsigned long p3 = 2; //speaker pulse thingamajig
const unsigned long p4 = 100; //adjust frequency for speaker
const unsigned long p5 = 1; //combiner
...
enum FRQ_State {FRQ_Start, FRQ_Wait, FRQ_Up, FRQ_Down};
int Tick_FRQ(int state);
...
int main(void) {
	...
	//task 4
	tasks[i].state = FRQ_Start;
        tasks[i].period = p4;
        tasks[i].elapsedTime = tasks[i].period;
        tasks[i].TickFct = &Tick_FRQ;
        i++;
...
int Tick_FRQ(int state){
	unsigned char temp = ~PINA;
	switch(state){
		case FRQ_Start: state = FRQ_Wait; break;
		case FRQ_Wait: 
			if (temp == 0x01)
				state = FRQ_Up;
			else if (temp == 0x02)
				...
				state = FRQ_Wait;
			break;
		case FRQ_Up: state = FRQ_Wait; break;
		case FRQ_Down: state = FRQ_Wait; break;
		default: state = FRQ_Start; break;
	}

	switch(state){
		case FRQ_Up: tasks[2].period += 1; break;
		case FRQ_Down: if (tasks[2].period > 1) tasks[2].period -= 1; break;
		default: break;
	}
	return state;
}
